...
public:
    ...
    friend CircularIterator operator+(const CircularIterator& it, const int n) {
        CircularIterator result = it;
        result += n;
        return result;
    }

    friend CircularIterator operator+(const int n, const CircularIterator& it) {
        return it + n;
    }
    ...
    friend int operator-(const CircularIterator& it1, const CircularIterator& it2) {
        return it1.index_ - it2.index_;
    }
};
...
public:
    ...
    void insertIt(iterator&& it, T element) {
        if (it > end())
            std::cout << "Insert after end!" << std::endl;
        else {
            insertEnd(*(end() - 1));
            iterator temp = end() - 2;
            while (temp != it) {
    ...
    void deleteIt(iterator&& it) {
        if (it >= end())
            std::cout << "Delete after end!" << std::endl;
        else {
            while (it != end() - 1) {
                *it = *(it + 1);
                it++;
    ...
    void changeCap(int value) {
        if (value < size_)
            std::cout << "Too little space!" << std::endl;
        else {
